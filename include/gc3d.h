...
public:
    ...
    */
    ...
    /**
      * @brief 函数 saveFringeFrames 将最近一次扫描的结构光条纹帧保存为 BMP 文件
      * @param [in] dir 输出目录 (默认 "debug_frames")
      * @return 保存成功返回 GC3D_SUCCESS，无保留数据返回 GC3D_1C1P_ERROR_METADATA_NONE
      * @note 帧文件命名为 frame_00.bmp ~ frame_<N-1>.bmp，N 为投影图案数量(18 或 19)
    */
    uint32_t saveFringeFrames(const std::string dir = "debug_frames");
    /**
    ...
    */
    ...
    /**
      * @brief 函数 getConvertDepthData 获取未经零平面转换的原始深度数据(mm)
      * @param [out] dst 深度数据缓冲区（调用方分配，大小为 imgW*imgH 的 float 数组）
      * @note  该深度图为相机坐标系下原始 z 值（16-bit 精度），未经 zeroPlaneHeight 翻转
      *         区别于 depthImageData（uint8 归一化图像），此数据保留度量尺度
      */
    void getConvertDepthData(float* dst);
    /**
